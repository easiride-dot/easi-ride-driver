  ...
  useEffect(() => {
    ...
    if (
      ...
      if (distToPickup <= PICKUP_ARRIVE_THRESHOLD) {
        transitionStatus(r.id, "driver_arrived", { arrived_at: new Date().toISOString() });
      }
    }
    ...
    if (
      ...
      if (distToPickup > PICKUP_DEPART_THRESHOLD) {
        transitionStatus(r.id, "in_progress", { in_progress_at: new Date().toISOString() });
      }
    }
    ...
    if (
      ...
      if (distToDest <= DESTINATION_ARRIVE_THRESHOLD) {
        transitionStatus(r.id, "completed", { completed_at: new Date().toISOString() });
      }
    }
